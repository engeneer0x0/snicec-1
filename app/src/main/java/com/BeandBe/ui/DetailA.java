...
public class DetailA extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		indice_TV = (TextView) findViewById(R.id.text_indice);
		...
		float pop = Float.parseFloat(production);
		float k= 15;
		float sur = Float.parseFloat(surfaceAgricole);
		
		float indiceK = (pop)/(sur*k) ;
		...
		if(Float.parseFloat(climat) == 0){
		climat_TV.setText("Climat : peux Vulnérable");
		}else{
			
			climat_TV.setText("Climat : très Vulnérable");
		}
		...
		indice_TV.setText("indice : " + String.valueOf(indiceK));
